...
fn main() {
    ...
    println!("Writing data in page: {}", page.num);
    dal.write_page(&page).unwrap_or_else(|err|{
        panic!("Error writing page: {err}")
    });
    println!("Saving freelist");
    ...
    dal2.write_page(&page).unwrap_or_else(|err|{
        panic!("Error writing page: {err}")
    });
    ...
    dal2.write_freelist().unwrap_or_else(|err|{
        panic!("Error writing freelist: {err}")
    });
